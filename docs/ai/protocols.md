...
Outlook AddIn 正式 protocol 是 SignalR-only：
...
Hub 不提供 AddIn HTTP long-poll / push channel；工作機 AddIn 不應呼叫 `/api/outlook/poll` 或 `/api/outlook/push-*`。
...
這些 endpoint 是 Web UI、AI 或 MCP client 對 Hub 發起 Outlook 工作的唯一使用者資料入口。HTTP caller 不需要理解內部執行端或 command；外部模型只有兩步：

1. 呼叫 `POST /api/outlook/request-*` 要求 SmartOffice 做一件事。
2. 用回傳的 `requestId` 持續呼叫 paired `POST /api/outlook/fetch-result-*`，直到 `state=completed`、`failed`、`unavailable` 或 `timeout`。所有 folders、mails、rules、categories、calendar、attachments、conversation 與 address book result 都從 `fetch-result-*` 的 `data` 讀取。

每個 `request-*` 都用固定 envelope 回傳 `requestId`、`request`、`state`、`message`、`data`。`data.fetchResultEndpoint` 直接指出下一步要呼叫的 paired endpoint；其他 `data` 欄位是各 request 自己的 struct，例如 mail search 的 `searchId`。
...
HTTP API 對外的 folder path 使用 `/主要信箱 - User/收件匣`。任何內部 path 轉換都是 Hub 實作細節，不應出現在外部 SKILL、Swagger 說明或 Web UI 文案中。
...
- `POST /api/outlook/request-mail-search`：建立 mail search operation；Hub 必須先確保 folder data 可用、展開 store/folder scope、切成單 folder slices，並節流執行。搜尋由 Outlook 內建搜尋執行，條件包含文字搜尋與篩選條件。
- `POST /api/outlook/request-mail-conversation`：建立單封郵件所屬 Outlook conversation 載入 operation；結果應包含同一討論串的 mail metadata，`includeBody=true` 時可一併包含 body/bodyHtml，方便 Web UI 一次性檢視討論串。
- `MailItemDto.messageClass` 必須保留 Outlook `MessageClass`。一般郵件是 `IPM.Note`；收件匣裡可開啟的會議邀請/更新通常是 `IPM.Schedule.Meeting.*`。Hub/Web UI 可讀取這類 item 的列表、body 與附件；`request-move-mail(s)` / `request-delete-mail` 可處理可移動的 Outlook item；`request-update-mail-properties` 只適用一般郵件，若 Hub 已知 item 不是 `IPM.Note`，API 應回 `unsupported_outlook_item_type`。
...
- `POST /api/outlook/request-signalr-ping`：建立 Outlook connection ping 測試 operation。
- `POST /api/outlook/request-calendar`：建立 Outlook calendar fetch operation。
- `POST /api/outlook/request-calendar-rooms`：建立 Outlook room/resource 清單 fetch operation；結果供 Web UI 下拉選單使用。
- `POST /api/outlook/request-create-calendar-event`：建立 SmartOffice-owned Outlook calendar event；people attendees 放在 `requiredAttendees[]`，會議室/設備放在 `resources[]`，Hub/AddIn 會寫入 ownership marker。
- `POST /api/outlook/request-update-calendar-event`：更新 SmartOffice-owned Outlook calendar event；request 必須帶 `eventId` 與 `smartOfficeEventId`，people attendees 放在 `requiredAttendees[]`，會議室/設備放在 `resources[]`，非 SmartOffice-owned event 或 `smartOfficeEventId` 不相符應回 `not_smartoffice_owned`。
- `POST /api/outlook/request-delete-calendar-event`：刪除 SmartOffice-owned Outlook calendar event；request 必須帶 `eventId` 與 `smartOfficeEventId`，不得刪除 Outlook 既有會議、他人邀請或使用者手動建立的 appointment。
- `POST /api/outlook/request-address-book`：建立 Outlook 通訊錄同步 operation；必須分批枚舉、限制 `maxGroupMembers` 與 `maxGroupDepth`，回傳 metadata-only 的 `AddressBookContactDto`。`maxContacts=0` 與 `maxAddressEntriesPerList=0` 表示不限制筆數；預設應載入完整通訊錄，並以 `maxGroupMembers=0` 避免在 Outlook UI 執行緒上大量展開 GAL group。通訊錄 entry 可能是個人、shared mailbox、distribution list 或 group；group 應以 `isGroup`、`memberCount`、有限的 `memberSmtpAddresses` 與 `memberGroupSmtpAddresses` 摘要呈現，不要一次展開無限制成員。
- `POST /api/outlook/request-address-book-group-members`：手動展開單一通訊錄 group 的 direct members。Hub 會 cache 已展開的 group，重複要求同一 group 時先回 cache；nested group 只標示為 `isGroup=true`，必須由 caller 再手動要求該 nested group，不得自動遞迴展開整棵 group tree。
...
- `POST /api/outlook/request-delete-folder`：建立 `delete_folder` operation；語意是將 folder 移到 Outlook default Deleted Items folder，不可永久刪除，也不可用顯示名稱或本地化名稱猜目的 folder。
- `POST /api/outlook/request-move-mail`：建立移動單封郵件 operation。
- `POST /api/outlook/request-move-mails`：建立移動多封郵件 operation；`mailIds` 必須來自 paired `fetch-result-*` 的 `data.mails`，單次最多 500 封。
- `POST /api/outlook/request-delete-mail`：建立 `delete_mail` operation；語意是移到 Outlook default Deleted Items folder，不可永久刪除，也不可用顯示名稱或本地化名稱猜目的 folder。
...
- `GET /api/outlook/mail-search/progress/{searchId}`：診斷用，查詢 mail search 進度；一般 client 仍以 paired `fetch-result-mail-search` 為主。
- `GET /api/outlook/mail-search/progress/by-command/{commandId}`：診斷用，以內部 command id 查詢 mail search 進度。
- `GET /api/outlook/address-book/lookup?email=...`：輕量檢查收件者 email 是否已出現在 SmartOffice 已知互動或已同步通訊錄裡；需要刷新資料時仍使用 `request-address-book` / `fetch-result-address-book`。
...
如果 Outlook connection 暫時不可用，request endpoint 仍回傳：
...
目前郵件已讀、flag 與 category mutation 以 `request-update-mail-properties` / `update_mail_properties` 為正式入口。Hub 公開 API 不提供 marker-style endpoint；文件不應要求工作機 AddIn 維護相容 handler。

## Hub Mail Search Planning

Hub 是 mail search 的負載控管者。Hub 收到 `request-mail-search` 時必須：
...
2. 若 folder data 無法建立，讓原始 search operation 失敗並回 `folder_unavailable` 或等價的使用者可理解錯誤。
...
- 單一 folder slice 內依 Microsoft Outlook `AdvancedSearch` / DASL 這類內建搜尋流程組合 filter，再回傳符合條件的 metadata。
- 同一個 folder slice 內可以分段回傳結果，每批約 `3` 到 `5` 封 mail metadata，最後一批才標 `isSliceComplete=true`。
...
任何新功能都要先問：能否用少量 request 表達使用者意圖，並由 Hub 控制 pagination、slice、batch 與 progress。除非 Outlook object model 本身要求逐項處理，Hub 不應一次啟動數百或數千個 Outlook 工作。

設計原則：

- 使用者發出一個操作時，HTTP API 儘量對應一個 parent request；需要多 folder 或多 item 時，由 Hub 規劃 slice/batch 與節流。
- Outlook 執行端只處理目前 request 的 Outlook object model 工作，不負責跨 request 排程、跨 folder 負載管理或 Web UI state 合併。
...
- 若真實 Outlook API 只能逐項處理，Hub contract 也要保留批次 request 的語意，在單一 request 內逐項處理並回報 progress/result，而不是由 Hub 對每一項各送一個 request。
...
- `PushAddressBookGroupMembersBatch(batch)`：回推單一 address book group 的 direct members。Hub 會依 group key cache 結果並 merge member metadata；不得在 AddIn 端自動遞迴展開 nested group。
