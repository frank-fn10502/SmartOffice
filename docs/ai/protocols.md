...
- `POST /api/outlook/request-update-calendar-event`：更新 SmartOffice-owned Outlook calendar event；people attendees 放在 `requiredAttendees[]`，會議室/設備放在 `resources[]`，非 SmartOffice-owned event 應回 `not_smartoffice_owned`。
- `POST /api/outlook/request-delete-calendar-event`：刪除 SmartOffice-owned Outlook calendar event；不得刪除 Outlook 既有會議、他人邀請或使用者手動建立的 appointment。
