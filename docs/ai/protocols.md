...
- `POST /api/outlook/request-address-book-relation`：AI、MCP client 與人類 HTTP caller 的主要通訊錄入口，用來查 group / person 關聯、direct members、nested groups、上層 groups，以及 group 收件者路徑對自己的 `recipientRelevance` 參考分數。Caller 只需要送出查詢條件並輪詢 paired `fetch-result-address-book-relation`，不需要理解通訊錄來源、內部載入或結果保存方式。
- `POST /api/outlook/request-address-book-roots`：Web UI 或目錄瀏覽型 caller 用來列出可瀏覽的通訊錄來源。
- `POST /api/outlook/request-address-list-entries`：Web UI 或目錄瀏覽型 caller 用來分頁瀏覽單一通訊錄來源的人員與 group entries。
- `POST /api/outlook/request-address-book-group-members`：Web UI 或目錄瀏覽型 caller 用來展開單一 group 的 direct members；不得在沒有使用者意圖時自動遞迴展開 nested group。
...
- `GET /api/outlook/address-book/lookup?email=...`：Web UI 輕量檢查收件者 email 的已知關聯；需要查 group 或 person 關係時使用 `request-address-book-relation`。
