...
- `POST /api/outlook/request-address-book`：建立 Outlook 通訊錄同步 operation；必須分批枚舉、限制 `maxContacts` 與 `maxAddressEntriesPerList`，回傳 metadata-only 的 `AddressBookContactDto`。
