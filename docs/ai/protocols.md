...
- `POST /api/outlook/request-address-book`：建立 Outlook 通訊錄 operation；外部 caller 只使用這個 request 與 paired `fetch-result-address-book`。通訊錄來源 discovery、指定來源 entries、下一頁與 group direct members 都由 Hub 依 request 狀態與使用者操作控制，不暴露成外部 workflow。不得把整份 GAL 當成啟動流程的一部分，也不得自動遞迴展開 nested group。
...
- `GET /api/outlook/address-book/lookup?email=...`：輕量檢查收件者 email 是否已出現在 SmartOffice 已知互動或已載入的通訊錄資料裡；需要刷新 Outlook 通訊錄時使用 `request-address-book` / `fetch-result-address-book`。
