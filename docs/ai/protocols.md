...
- `POST /api/outlook/request-create-calendar-event`：建立 SmartOffice-owned Outlook calendar event；Hub/AddIn 會寫入 ownership marker。
- `POST /api/outlook/request-update-calendar-event`：更新 SmartOffice-owned Outlook calendar event；非 SmartOffice-owned event 應回 `not_smartoffice_owned`。
...
- `POST /api/outlook/request-address-book`：建立 Outlook 通訊錄同步 operation；必須分批枚舉、限制 `maxContacts` 與 `maxAddressEntriesPerList`，回傳 metadata-only 的 `AddressBookContactDto`。通訊錄 entry 可能是個人、shared mailbox、distribution list 或 group；group 應以 `isGroup`、`memberCount` 與有限的 `memberSmtpAddresses` 摘要呈現，不要一次展開無限制成員。
