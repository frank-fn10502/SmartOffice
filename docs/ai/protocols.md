...
- `POST /api/outlook/request-address-book`：建立 Outlook 通訊錄同步 operation；必須分批枚舉、限制 `maxContacts`、`maxAddressEntriesPerList`、`maxGroupMembers` 與 `maxGroupDepth`，回傳 metadata-only 的 `AddressBookContactDto`。通訊錄 entry 可能是個人、shared mailbox、distribution list 或 group；group 應以 `isGroup`、`memberCount`、有限的 `memberSmtpAddresses` 與 `memberGroupSmtpAddresses` 摘要呈現，不要一次展開無限制成員。
