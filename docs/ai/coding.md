...
- `webui/src/` 內 `.ts` 與 `.vue` 檔案不得超過 800 行。這是硬性驗收規則，不是提醒或建議；新增或修改 Web UI 程式碼後，必須讓 `npm run check:file-lines` 或 `./scripts/build-in-container.sh` 通過。若檔案超過 800 行，當次任務必須先切分，不能只在回覆中說明「後續再處理」。
...
- 每次新增大量邏輯或樣式時，必須檢查受影響檔案行數與職責邊界；最終回報需包含 line-count gate 或 container build 的結果。若不是 Web UI 檔案、因此無法套用自動 gate，也要用 `wc -l` 檢查任務相關的大檔案。
