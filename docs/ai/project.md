...
本 repository 是開發機上的 `SmartOffice.Hub` 專案，負責 Hub contract、Web UI、mock Add-in 與文件。這台開發機不一定有工作機的完整 SmartOffice / Outlook AddIn solution。
...
- Outlook COM/VSTO automation、poll Hub、push folders/mails/rules/calendar、修改 mail/category/folder 等實作，必須在工作機 SmartOffice / Outlook AddIn 專案中完成。
