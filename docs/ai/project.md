...
- `wwwroot/`：靜態 Web UI，刻意避免 frontend build chain。
...
- `wwwroot` 內使用 static HTML/CSS/JavaScript。
- Prototype 階段使用 in-memory store。

除非任務明確需要，請不要引入 database、frontend build system、background job framework 或 AI SDK。
