...
- 每次 `request-*` 後，先解析 dispatch response 的 `operationId`；dispatch response 沒有 `success` 欄位，不可在這一步判斷已成功。
- 取得 `operationId` 後呼叫 `POST /api/outlook/operation-state`；回應會同時提供 `status`、`success`、`progress`、`metadata`、分頁 `items`、`nextCursor`、`hasMore` 與 `complete`。
- 使用者或 AI 只需要 loop `operation-state` 到 `complete=true`；需要資料時帶 `includeItems=true` 與 `cursor`，不要再猜下一個資料 endpoint。
- 若 `request-*` 回 HTTP 409 / 400 / 502 / 504，仍先解析 body 的 `status`、`message` 與可能存在的 `operationId`；不要靠猜測重試，也不要改用更大的搜尋範圍。
- 修改郵件前必須先從 `operation-state.items` 取得 `MailItemDto.id` 與 `folderPath`，不可只用 subject、sender 或 folder name 猜目標。
...
- `request-delete-mail` 的語意是移到 Outlook default Deleted Items folder，不是永久刪除；目的 folder 由 AddIn 用 Outlook default folder identity 定位，不靠顯示名稱猜測；仍需先確認 `mailId` 與 `folderPath` 來自 `operation-state.items` 或目前資料。若目標已經在 Deleted Items 內，API 會要求使用者自行到 Outlook 永久刪除。
- 使用者未指定 folder 時，只查主要 mailbox 的 Inbox；但 Inbox path 必須取自 `GET /api/outlook/folders`，不可硬寫英文 `Inbox`。
...
1. 確認 API status。
2. `POST /api/outlook/request-folders`，取得 `operationId` 後 loop `POST /api/outlook/operation-state` 到 `complete=true`。
3. 從 `operation-state.metadata.stores` 與 `operation-state.items` 找主要 store/root folder。
...
5. 再用 `operation-state` 讀 folder items，在主要 store 底下選 `folderType="Inbox"` 的 folder；若沒有，才用 `name` 等於 `收件匣` 或 `Inbox` fallback。
6. 使用該 folder 的完整 `folderPath` 呼叫 `request-mails` 或放入 `request-mail-search.scopeFolderPaths[0]`。
7. 取得 `operationId` 後 loop `operation-state`，從 `items` 讀資料。
8. 摘要必要 metadata；只有任務需要時才讀 body 或 attachment。
9. mutation 後用該次 `operation-state` 或重新送出必要 request 確認結果。

找不到主要 Inbox 時，停止並回報目前 folder 資料無法定位 Inbox；不要自行改成全信箱或空 scope 搜尋。

## 操作流程速查

- 最近郵件：Golden Path 取得 Inbox path -> `request-mails` -> `operation-state`。
- 指定 folder 內所有 mails：取得 folder path -> `request-folder-mails` -> `operation-state`。
- 郵件搜尋：Golden Path 取得 Inbox path -> `request-mail-search` 且 `scopeFolderPaths` 放該 path -> `operation-state`。
- 讀 body：先從 `mails` 或 `mail-search` 取 `id` 與 `folderPath` -> `request-mail-body` -> `mails` 找同 id 的 `body` / `bodyHtml`。
- 讀附件：先從 `operation-state.items` 取 `id` 與 `folderPath` -> `request-mail-attachments` -> `operation-state`，必要時讀 `mail-attachments?mailId={id}`。
- 修改、移動、刪除郵件：先從 `operation-state.items` 確認唯一目標的 `id` 與 `folderPath` -> mutation endpoint -> `operation-state`。
...
- `Inbox` 是範例名稱，不是穩定 contract；中文 Outlook 常見路徑是 `/主要信箱 - User/收件匣`。操作前一定要從 `GET /api/outlook/folders` 取實際 `folderPath`。
- `request-folders` 只保證載入 stores/root folders；若 root 的 `childrenLoaded=false`，要用 `request-folder-children` 展開後再找 Inbox。
- `request-folder-children` 的 request 欄位是 `storeId`、`parentEntryId`、`parentFolderPath`；值分別取自 `GET /api/outlook/folders` 回傳 root folder 的 `storeId`、`entryId`、`folderPath`。不要送 `entryId` 或 `folderPath` 這兩個錯誤欄位名，也不要只傳 folder display name。
- `request-mails.folderPath`、`request-folder-mails.folderPath` 與 `request-mail-search.scopeFolderPaths[]` 必須完整等於 `GET /api/outlook/folders` 裡的 `folderPath`。
...
- 不要自行組 folder path；一律使用 `GET /api/outlook/folders` 回傳的 `/Mailbox/Inbox` 形式。
...
- `request-*` response 是否足以提供 `operationId`，且 `operation-state` 是否足以表達狀態與資料頁。
- mutation endpoint 是否都要求穩定 id，而不是只靠顯示名稱。
- destructive 或本機檔案操作是否有足夠明確的識別與限制。
- mail search 的進度、結果與 operation state 是否能被 agent 清楚串起來。
