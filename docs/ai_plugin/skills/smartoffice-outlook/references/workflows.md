...
等 `state=completed` 後讀 `data.conversationTopic` 與 `data.mails[]`。若 `next.hasMore=true`，下一次 request 帶 `next.cursor`。回覆使用者時說明這是哪一個 conversation、共取得幾封 mail，並只摘要任務必要內容；不要貼出完整討論串或大量 mail body。
