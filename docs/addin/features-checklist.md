...
| 搜尋郵件 | Search panel / AI mail lookup | `search_mails` | `BeginMailSearch`、`PushMailSearchBatch`、`CompleteMailSearch` |
...
| 刪除郵件 | Mail header delete button | `delete_mail` | `PushMails`、folder 增量同步；實作為 Move to Deleted Items |
...
Web UI 與 AI 使用同一個 `search_mails` command 查找郵件。AddIn 必須把搜尋視為低衝擊背景工作，避免造成 Outlook UI 卡頓。

- [ ] AddIn 收到 `search_mails`。
- [ ] 一次只處理一個 Outlook search job；新搜尋可排隊或讓較舊搜尋結果失效。
- [ ] 優先使用 `searchMailsRequest.storeId` 限定單一 Outlook Store；全域搜尋必須由 AddIn 依 store / folder 分批查找。
- [ ] 支援 `scopeFolderPaths` 與 `includeSubFolders`；有 folder scope 時應先縮小搜尋範圍。
- [ ] 支援 `receivedFrom` / `receivedTo` 時間區段。
- [ ] 支援 `exactReceivedTime`，用 `exactReceivedToleranceSeconds` 當容忍秒數，不要期待 Outlook timestamp 永遠精準到秒。
- [ ] 支援 `keyword` 的 `contains` 與 `exact`；`regex` 只能在 bounded result 內後篩，不得因 regex 對大型 store 做全量 body 掃描。
- [ ] 支援 `fields`：`subject`、`sender`、`categories`、`body`；body search 應在時間、folder、store、maxCount 約束下執行。
- [ ] `maxCount` 必須有上限，建議 AddIn 端 clamp 到 200 以內。
- [ ] 回推結果只包含 mail metadata，`body` / `bodyHtml` 留空；使用者點開後才走 `fetch_mail_body`。
- [ ] 使用 `BeginMailSearch`、`PushMailSearchBatch`、`CompleteMailSearch` 分批回推，不要用 `PushMails` 覆蓋目前 folder list。
- [ ] 發生 Outlook busy、search timeout 或使用者取消時，仍必須 `ReportCommandResult` 並以匿名化 message 說明。

驗收：

- [ ] Web UI 可用時間區段搜尋。
- [ ] Web UI 可用單一時間加容忍秒數搜尋。
- [ ] Web UI 可用片段 keyword 搜尋 subject / sender。
- [ ] Web UI 指定目前 folder 或目前 store 時，結果範圍正確縮小。
- [ ] 全域搜尋會分批更新結果，而且 Outlook 仍可操作。
- [ ] Regex 模式在大型範圍不會觸發長時間全量掃描。
...
刪除郵件有獨立 `delete_mail` command，方便 Web UI 測試 AddIn 與 Hub 的刪除語意；但唯一允許實作仍是移動到 Outlook 的「刪除的郵件 / Deleted Items」folder。AddIn 不得直接呼叫 `MailItem.Delete()` 或永久刪除郵件。
