...
- [ ] Hub folder snapshot 只保留 store root 與 Hub 判定可操作的 mail folder；日誌、記事、Calendar、Contacts、Tasks、Sync Issues、Local Failures、Server Failures 等不可操作 folder 不應出現在 Web UI folder list。
...
- [ ] AddIn 收到 `move_mail`。
- [ ] AddIn 收到 `move_mails` 時，逐封用 `moveMailsRequest.mailIds` 找回 mail item；Hub 單次最多 dispatch 500 封，更多郵件由 caller 分批呼叫。
...
- [ ] 若 source mail 已經位於 default Deleted Items folder 或其子層，不得進一步刪除；回報 `manual_delete_required` 或等效診斷，請使用者自行到 Outlook 永久刪除。
...
- [ ] 若目標是 store root、hidden/system folder、default Deleted Items folder 本身，或已經位於 default Deleted Items folder 底下，不得永久刪除；應回報失敗或 no-op 診斷訊息。
- [ ] 若目標已經位於 default Deleted Items folder 或其子層，回報 `manual_delete_required` 或等效診斷，請使用者自行到 Outlook 永久刪除。
