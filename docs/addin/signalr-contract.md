...
- `isSearchableMailFolder`: boolean，AddIn 只回報 Outlook 事實：`defaultItemType == 0` 且 `isHidden == false` 且 `isSystem == false` 且不是 store root 時為 `true`；Hub 仍是最終 search scope filter。
...
`FolderDto` 不再包含 `subFolders`，也不再重複保存 store display name / type / file path。tree 由 `parentFolderPath` 與 `storeId` 組回。Folder discovery 由 Hub 主導；AddIn 不得保留全量 folder tree command。Hub folder cache 只保留 store root 與可操作 mail folder；`defaultItemType != 0`、`isHidden == true`、`isSystem == true`、`isSearchableMailFolder == false` 的非 root folder 不會進入 Web UI folder list、folder command scope 或 search planning。AddIn 不得用本地化 folder name 猜測是否為系統資料夾。
