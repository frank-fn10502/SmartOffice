...
AddIn 應用 `mailId` 與 `folderPath` 找到 Outlook item，再以該 item 所在 store / mailbox 的 default Deleted Items folder 作為 destination，呼叫 Outlook `MailItem.Move(destinationFolder)`。目的 folder 必須透過 Outlook default folder identity 定位，例如 `Store.GetDefaultFolder(olFolderDeletedItems)` 或等效流程；不可用顯示名稱、本地化名稱、`folderPath` 字串或 Web UI 顯示文字猜測。完成後回推最新 `PushMails`，並用 folder 增量同步更新 source folder 與 Deleted Items folder count。這個 command 不得呼叫 Outlook `MailItem.Delete()`。
