...
所有 mail / folder delete 類 command 都是 soft delete。若目標已經位於 Outlook default Deleted Items folder 或其子層，AddIn 與 API 都不得進一步刪除；應回報 `manual_delete_required` 或等效失敗 / no-op 訊息，請使用者自行到 Outlook 永久刪除。
...
AddIn 應用 `folderPath` 找到 Outlook folder object，再把該 folder move 到 default Deleted Items folder。若目標是 store root、hidden/system folder、default Deleted Items folder 本身，或已經位於 default Deleted Items folder 底下，AddIn 不得永久刪除；應回報失敗或 no-op 診斷訊息。完成後用 folder 增量同步回推 source parent、Deleted Items folder 與被移動 folder tree 的最新狀態；若目前 mail list 指向被移動 folder，也要回推 `PushMails` 清掉或更新畫面。
...
若 `destinationFolderPath` 解析到 Outlook default Deleted Items folder，這仍然只是移動郵件到該 folder，不是永久刪除。AddIn 必須沿用同一個 `MailItem.Move(destinationFolder)` 流程，不可因目的 folder 是 Deleted Items 而改呼叫 `MailItem.Delete()`。若 source mail 已經位於 default Deleted Items folder 或其子層，不得進一步刪除，應回報請使用者自行到 Outlook 永久刪除。
...
AddIn 應用 `mailId` 與 `folderPath` 找到 Outlook item，再以該 item 所在 store / mailbox 的 default Deleted Items folder 作為 destination，呼叫 Outlook `MailItem.Move(destinationFolder)`。目的 folder 必須透過 Outlook default folder identity 定位，例如 `Store.GetDefaultFolder(olFolderDeletedItems)` 或等效流程；不可用顯示名稱、本地化名稱、`folderPath` 字串或 Web UI 顯示文字猜測。若 source mail 已經位於 default Deleted Items folder 或其子層，不得進一步刪除，應回報請使用者自行到 Outlook 永久刪除。完成後回推最新 `PushMails`，並用 folder 增量同步更新 source folder 與 Deleted Items folder count。這個 command 不得呼叫 Outlook `MailItem.Delete()`。
...
| `POST` | `/api/outlook/request-folder-mails` | Hub 規劃後 dispatch 多個 `fetch_mail_search_slice` |
| `POST` | `/api/outlook/request-mail-search` | Hub 規劃後 dispatch 多個 `fetch_mail_search_slice` |
...
- `displayName`: string，Web UI 與 client 預設顯示的名稱；應使用 Outlook resolved recipient / address book 的直觀名稱。
...
`FolderDto` 不再包含 `subFolders`，也不再重複保存 store display name / type / file path。tree 由 `parentFolderPath` 與 `storeId` 組回。Folder discovery 由 Hub 主導；AddIn 不得保留全量 folder tree command。Hub folder cache 只保留 store root 與 Hub 判定可操作的 mail folder；`defaultItemType != 0`、`isHidden == true`、`isSystem == true`，或 `folderType` 是 blocked enum 的非 root folder，不會進入 Web UI folder list、folder command scope 或 search planning。AddIn 不得用本地化 folder name 猜測是否為系統資料夾。
