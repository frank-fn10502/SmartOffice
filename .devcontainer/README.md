...
這個資料夾提供「可選」的 devcontainer 開發環境。它不取代本機快速開發流程，只是在你需要完整 container 開發環境時提供一致的 .NET 8 SDK 與 VS Code 設定。

## 目前設計

- `devcontainer.json` 使用本資料夾內的 `Dockerfile`。
- `Dockerfile` 目前只建立 .NET 8 devcontainer baseline，尚未安裝額外套件。
...
這個腳本會在需要時先用 `.devcontainer/Dockerfile` 建立 reusable local image，然後用 `docker run --rm` 開一個暫存 container 執行 `dotnet build`。也就是說，image 會保留以加速後續編譯，但每次實際編譯用的 container 不會常駐。
